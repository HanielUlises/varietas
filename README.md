...
**Documentation:** https://hanielulises.github.io/varietas/ for the theory, the user guide, and the API reference. Build it locally with `sphinx-build -b html docs docs/_build/html`.
...
Complex and real points are reported separately, a residual is available to certify any point, and every failure mode is named, whether empty variety, positive dimension or a defective eigenstructure, rather than returning a quietly truncated solution set.
...
`varietas_codegen` is the offline half of the library. It supplies the field the Gröbner computation actually runs over, namely `varietas::rational`, arbitrary precision rationals backed by GMP, together with the `coefficient_traits` specialisation that the core algorithms consult, and, above it, `rational_function<P>`, which adjoins the pose to that field so a basis can be computed once for every pose rather than once per pose. `emit` writes the result out as a header. The same Buchberger implementation runs over all three fields without modification, which is the parameterisation paying for itself twice.

Nothing in `varietas_core` changes, since the ideal, quotient and solving layers were already templated on the coefficient type, so the same Buchberger implementation runs over $\mathbb{Q}$ offline and over `double` at runtime, and the two are exercised by the same test bodies instantiated twice.
...
What it deliberately is not is `matrix3<Coeff>`: that type asks `coefficient_traits` for inverses, and polynomials are not invertible, so the substitution needs its own type. That is the type system saying something true rather than an inconvenience.
...
The exercise also measured something about the formulation. Eliminating for the torus takes seventy seconds. Presenting the *same* arm with two variables per joint and the relation $c^2+s^2=1$, which has no denominators and therefore needs no saturation at all, eliminates in sixty milliseconds and returns the identical quartic, a factor of a thousand. The half-angle substitution buys one variable per joint rather than two, which is what matters for the zero-dimensional inverse kinematics the library is built around, and pays for it with a denominator, which is what matters here. The two formulations are not competitors so much as tools for different questions, and implicitization has been moved onto the second.
...
That comparison is a test rather than a remark. Both paths are run on the torus arm and their reduced bases are required to be equal, which is what licenses the migration: the elimination ideal does not depend on the parameterisation it was computed through, only the cost does. The same test file checks the two forward kinematics maps against each other exactly, since the correspondence $c=(1-t^2)/(1+t^2),\ s=2t/(1+t^2)$ carries a rational $t$ to a rational point of the circle, so a configuration is expressible in both formulations without leaving $\mathbb{Q}$ and the maps must agree identically rather than nearly, and checks that on the circle the composed rotation is exactly orthogonal, the admissible defect over $\mathbb{Q}$ again being zero and not a threshold.
...
| Pose inverse kinematics at a given target | complete, to five joints |
| Modular gcd over $\mathbb{Q}(\boldsymbol{p})$ | coprimality test only; see `doc/parametric_cost.pdf` |
...
It refuses more than it accepts, which is the point, and the counts settle most of it before any Gröbner basis is attempted. Fewer pose coordinates than unknowns cannot give a finite solution set: $P$ polynomials in $N$ variables cut out components of dimension at least $N-P$, and saturation only removes components. More coordinates than unknowns cannot give a nonempty one: the positions an $N$-joint arm reaches form a variety of dimension at most $N$, the parameters are transcendentals rather than a point of that variety, and a general pose is simply out of reach, and the ideal is the unit ideal. So $P=N$ is the only arrangement that can produce a solver at all, and both other cases are rejected by counting. A coordinate left out of the parameter list is separately checked to be identically zero before its equation is dropped, since dropping an equation that constrains the joints would silently answer about a larger variety. What counting cannot catch, two joints turning about one axis for instance, the quotient dimension does.
...
Two things that were once wrong here are worth recording, because the tests that pin them down read oddly without the history. The emitted guard used to compare denominators against zero exactly, which is the wrong question in floating point: a denominator that vanishes mathematically almost never evaluates to `0.0`, it evaluates to whatever the cancellation between its terms leaves behind. On the planar 2R arm the pole is the circle $x^2+y^2+2x=0$, an artefact of the elimination rather than anything the arm cannot reach, and at $(-1.6, 0.8)$, which is on it exactly over $\mathbb{Q}$, the same expression in doubles comes to about $4\times10^{-16}$. The guard did not fire, the matrices were formed by dividing by that, and one of the two returned branches did not reach the target, with neither the count nor the status saying so. The guard now compares each denominator against the sum of the magnitudes of the terms that produced it, so total cancellation is recognised whether or not it landed on zero, and a pose merely near the pole is still answered. Monomial exponents were held in a `uint8_t`, and parameter polynomials over a function field reach that ceiling (degree 254 in one variable was observed in an underdetermined system), after which a product wrapped to a different monomial and a polynomial that divided another silently stopped dividing it. Exponents are sixteen bits now, and the product asserts rather than wraps, which is the half that matters.

The third is scale, and it is the one that decides what this pipeline is for. The cost is not in the arm but in the number of parameters adjoined. The planar 2R arm solves over $\mathbb{Q}(x,y)$ in about thirty milliseconds. The anthropomorphic 3R arm, base yaw about $z$ with shoulder and elbow pitching about $y$, solves over $\mathbb{Q}$ at a fixed pose in four milliseconds, returning a basis of six elements and $\dim_k A = 4$, which is exactly the four branches such an arm is known to have; over $\mathbb{Q}(x,y,z)$ the same system had produced no answer after fifteen minutes and was abandoned. The arm is not the difficulty.

The difficulty is cancellation. `rational_function` normalises after every coefficient operation, and the polynomial gcd that cancellation needs is what the run is made of: sampled over three minutes, about eighty-six per cent of the time is spent inside `polynomial_gcd`, and the cost per call grows sharply as the parameter polynomials do: the first twenty-seven thousand calls take five seconds between them, while a later sixteen hundred take sixty-eight. Cancellation cannot simply be dropped, since without it the entries grow past any use.

Most of those calls return 1, so normalisation now asks a cheap question first. Specialising every parameter but one at fixed values in a small prime field leaves two univariate polynomials whose gcd is a Euclidean algorithm on machine integers; a constant answer there is strong evidence of coprimality, and the exact gcd is computed only when it is not. Every variable has to be kept in turn, since a factor involving only the specialised variables collapses to a constant and would be missed. The test is evidence rather than proof, and that is admissible precisely because it only ever decides whether to *skip* a cancellation: a skipped one leaves the fraction in higher terms, which costs size and never correctness.
...
Run against a real robot, the pipeline mostly declines, and the way it declines is the useful part. The KUKA iiwa in `varietas_urdf/test/data` recovers exactly, with the audit moving no joint by more than $5\times10^{-12}$ radians, and then has seven joints, which is four more than a tool position can constrain, so `urdf_codegen` says so and stops. Truncating the chain does not rescue it either: taking the tip to be `lbr_iiwa_link_3` gives three joints, but that link's frame is the third joint's own frame, so the tool sits on the axis that joint turns about and cannot be moved by it. The position problem is a two-joint problem wearing three joints, the reduced system comes out positive-dimensional, and the dimension check catches it in about half a second. There is no link boundary on this arm where a well-posed three-joint positioning problem appears. That is a fair summary of the present reach of the parametric path: it is a tool for small subsystems, not for a seven-axis manipulator, and the thing it does well is refuse promptly and say which of the three reasons applies.

What none of these timings say is what a *caller* pays, and that is the number the whole arrangement exists to make small. `doc/experiments/solver_cost.cpp` measures it on the three-joint arm: one call to the emitted `branch_ik::solve` takes about 0.9 microseconds at the median and 1.3 at the 99th centile, which is two and a half evaluations of the same arm's forward kinematics, or a little over a million solves a second on one core. Generating the header cost 0.35 seconds, once, offline; that is some four hundred thousand solves, or six minutes of a 1 kHz loop, and it is paid by the build rather than by the caller.

The comparison worth making is against the thing a codebase reaches for when it has no closed form. A damped least squares iteration on the same arm takes about twenty times as long as one generated solve, converges from about ninety-one per cent of random seeds, and when it converges returns one configuration: whichever the seed fell into, with no way to say how many others exist. Recovering the whole set numerically means restarting it, and over three hundred targets that took about ten seeds each, missed a branch outright on one of them within a budget of sixty, and cost some 370 microseconds per target against 0.9 for the single generated call. Four hundred times the cost, for an answer that is still a hope rather than a count. The accuracy is not the trade either: the median residual over 71,658 returned configurations is $3\times10^{-16}$ m, though the tail reaches $6\times10^{-8}$ m on about one in five thousand, which is a real and so far unexplained blemish rather than the boundary or singularity effect it first looks like.

#### One pose at a time

Orientation cannot be adjoined. A general pose is six parameters, since twelve matrix entries are not independent and a general point of $\mathbb{A}^{12}$ is not a rigid motion at all, and two adjoined parameters is the working limit. So there is no parametric solver for a full pose and there is not going to be one at this size. Giving up the parameters is what buys orientation back: with the target a constant of $\mathbb{Q}$ rather than a parameter of $\mathbb{Q}(\boldsymbol{p})$, the twelve equations of a pose are no harder for Buchberger than the three of a position, and `varietas_kinematics` has posed them since the beginning. `urdf_solve` is that path as a command.

It is worth stating what it costs, because the growth is steep and it is the whole story: on a chain of unit links, a full pose takes about ten milliseconds at three joints, a sixth of a second at four, nine seconds at five, and had produced nothing after eight minutes at six. Five joints is where this stops being interactive, and six is where it stops.

The pose has to be exact, and that is not a formality either. A target assembled by rounding sines and cosines into rationals carries denominators near $2^{52}$, and a run over coefficients that size does not finish. The first attempt at a six-joint pose here failed that way before it failed for any interesting reason. So the target is snapped onto a nearby exact pose by the same quaternion argument the URDF front end uses, and the deviation is reported rather than hidden. Right angles survive that snapping exactly, which is what makes it usable rather than merely defensible.

The last thing worth knowing is that asking for a full pose from an arm with too few joints is not a harmless over-specification. The poses a three-joint arm can strike form a three-dimensional subvariety of a six-dimensional space, so a target moved even slightly off it is unreachable in the strict sense: the ideal becomes the unit ideal and the answer is no configuration at all, rather than a configuration that nearly works. `urdf_solve` says so in those words. Asking such an arm for a position instead returns the four postures it genuinely has: the same four the decoupled generated solver returns, by an entirely different computation, which is the cross-check the two paths give each other.
